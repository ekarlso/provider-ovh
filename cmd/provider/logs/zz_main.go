...
func main() {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-ovh-logs",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_logs(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_logs(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_logs(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_logs(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	}
