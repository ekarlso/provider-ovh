...
import (
	...
	clusterapis "github.com/edixos/provider-ovh/apis/cluster"
	namespacedapis "github.com/edixos/provider-ovh/apis/namespaced"
	"github.com/edixos/provider-ovh/config"
	resolverapis "github.com/edixos/provider-ovh/internal/apis"
	"github.com/edixos/provider-ovh/internal/bootcheck"
	"github.com/edixos/provider-ovh/internal/clients"
	clustercontroller "github.com/edixos/provider-ovh/internal/controller/cluster"
	namespacedcontroller "github.com/edixos/provider-ovh/internal/controller/namespaced"
...
func main() {
	...
	logr := logging.NewLogrLogger(zl.WithName("provider-ovh"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-ovh-dedicatedserver",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_dedicatedserver(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_dedicatedserver(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_dedicatedserver(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_dedicatedserver(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	}
