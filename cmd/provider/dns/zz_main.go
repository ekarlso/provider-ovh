...
func main() {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-ovh-dns",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_dns(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_dns(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_dns(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_dns(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	}
