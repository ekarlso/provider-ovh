...
func main() {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-ovh-additionalip",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_additionalip(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_additionalip(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_additionalip(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_additionalip(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	}
